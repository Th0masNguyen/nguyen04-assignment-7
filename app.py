...
def hypothesis_test():
    ...
    if test_type == "!=":  # Two-tailed test
        p_value = (np.sum(np.abs(simulated_stats - hypothesized_value) >= np.abs(observed_stat - hypothesized_value))
                   / len(simulated_stats))
    elif test_type == ">":  # One-tailed test (greater)
        p_value = np.sum(simulated_stats >= observed_stat) / len(simulated_stats)
    elif test_type == "<":  # One-tailed test (less)
        p_value = np.sum(simulated_stats <= observed_stat) / len(simulated_stats)
    else:
        return "Error: Invalid test type selected.", 400

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    if p_value <= 0.0001:
        fun_message = "Wow, small p_value. Very rare. Very demure."
    else:
        fun_message = "Not fun. No Message."
    ...
    plt.axvline(observed_stat, color="red", linestyle="dashed", label="Observed Statistic")
    plt.axvline(hypothesized_value, color="blue", linestyle="dashed", label="Hypothesized Value")
    plt.xlabel("Simulated Statistic")
    plt.ylabel("Frequency")
    plt.legend()
    plt.title(f"Histogram of {parameter.capitalize()}s with Observed and Hypothesized Values")
...
def confidence_interval():
    ...
    t_critical = stats.t.ppf(1 - alpha / 2, df=S - 1)  # Two-tailed t critical value
    ...
    plt.scatter(estimates, np.zeros_like(estimates), color="gray", alpha=0.7, label="Simulated Estimates")
    plt.scatter([mean_estimate], [0], color="green" if includes_true else "red", label="Mean Estimate")
    plt.axvline(true_param, color="blue", linestyle="dashed", label="True Parameter Value")
    plt.hlines(y=0, xmin=ci_lower, xmax=ci_upper, color="black", linewidth=2, label="Confidence Interval")
